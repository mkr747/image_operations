...
class AlgorithmView:
    ...
    def create(self, window, width=300, height=900):
        ...
        self.__editIcon = Utils.read_iamge('assets/edit.png')
        self.__deleteIcon = Utils.read_iamge('assets/delete.png')
    ...
    def append(self, step: CommandEnum):
        ...
        moveUpButton = self.__create_move_button(
            self.__algorithm_frame, cmd.uuid, '^', row, 1, True)
        moveDownButton = self.__create_move_button(
            self.__algorithm_frame, cmd.uuid, '˅', row, 2, False)
    ...
    def __create_move_button(self, frame, uuid: UUID, text: str, row: int, column: int, up):
        return gf.create_button(frame, text, partial(self.move, uuid, up), row, column)
